...
class WP_Authress_Lock {
	...
	public function get_lock_options() {
		...
		$extraOptions = [
			'auth' => [
				'params' => [
					...
					'scope' => WP_Authress_LoginManager::get_userinfo_scope( 'lock' ),
				],
			],
		];

		$extraOptions['auth']['params']['nonce'] = WP_Authress_Nonce_Handler::get_instance()->get_unique();
		...
		if ( ! $show_as_modal ) {
			$options_obj['container'] = WP_AUTHRESS_AUTHRESS_LOGIN_FORM_ID;
		}
		...
		if ( wp_authress_is_current_login_action( [ 'register' ] ) ) {
			$options_obj['initialScreen'] = 'signUp';
		}

		return apply_filters( 'authress_lock_options', $options_obj );
	}
	...
	public static function render_back_to_lock() {
		...
		printf(
			...
			sanitize_text_field( sprintf( __( '← Back to %s login', 'wp-authress' ), $title ) )
		);
	}
	...
	public static function render( $canShowLegacyLogin = true, $specialSettings = [] ) {
		...
		if ( $canShowLegacyLogin && wp_authress_can_show_wp_login_form() ) {
			...
			add_action( 'woocommerce_after_customer_login_form', [ 'WP_Authress_Lock', 'render_back_to_lock' ] );
			return;
		}
