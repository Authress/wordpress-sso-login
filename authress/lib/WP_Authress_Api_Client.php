<?php
/**
 * Contains Class WP_Authress_Api_Client class.
 *
 * @package WP-Authress
 *
 * @since 1.2.1
...
class WP_Authress_Api_Client {
	...
	/**
	 * Basic header components for an Authress API call.
	 *
	 * @since 3.5.0
	 *
	...
	private static function get_headers( $token = '', $content_type = 'application/json' ) {
		...
		$headers = WP_Authress_Api_Abstract::get_info_headers();
	...
	public static function signup_user( $domain, $data ) {
		...
		if ( $response instanceof WP_Error ) {
			WP_Authress_ErrorLog::insert_error( __METHOD__, $response );
			return false;
		}

		if ( $response['response']['code'] !== 200 ) {
			WP_Authress_ErrorLog::insert_error( __METHOD__, $response['body'] );
			return false;
		}
	...
	public static function create_client( $domain, $app_token, $name ) {
		...
		$payload = [
			...
			'cross_origin_loc'    => add_query_arg( 'authressfallback', '1', site_url( 'index.php', 'https' ) ),
			'cross_origin_auth'   => true,

			// A set of URLs that are valid to redirect to after logout from Authress
			'allowed_logout_urls' => [
				home_url(),
				wp_login_url(),
			],
			...
			// "Use Authress to do Single Sign On"
			'sso'                 => true,
		...
		if ( $response instanceof WP_Error ) {
			WP_Authress_ErrorLog::insert_error( __METHOD__, $response->get_error_message() );
			return false;
		}
	 ...
	 * @param string $access_key - Client ID for the WordPress Application.
	 *
