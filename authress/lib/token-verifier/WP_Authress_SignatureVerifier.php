<?php
/**
 * Contains Trait WP_Authress_SignatureVerifier.
 *
 * @package WP-Authress
 *
 * @since 4.0.0
...
/**
 * Class WP_Authress_SignatureVerifier
 *
 * @codeCoverageIgnore - Classes are adapted from the PHP SDK and tested there.
 */
abstract class WP_Authress_SignatureVerifier {
	 ...
	 * @return Token
	 *
	 * @throws WP_Authress_InvalidIdTokenException If JWT format is incorrect.
	 * @throws WP_Authress_InvalidIdTokenException If token algorithm does not match the validator.
	 * @throws WP_Authress_InvalidIdTokenException If token algorithm signature cannot be validated.
	 */
	final public function verifyAndDecode( string $token ) : Token {
		try {
			$parsedToken = $this->parser->parse( $token );
		} catch ( InvalidArgumentException $e ) {
			throw new WP_Authress_InvalidIdTokenException( 'ID token could not be decoded' );
		}
		...
		if ( ! $this->checkSignature( $parsedToken ) ) {
			throw new WP_Authress_InvalidIdTokenException( 'Invalid ID token signature' );
		}
