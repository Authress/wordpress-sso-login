<?php
/**
 * Contains Trait WP_Authress_IdTokenVerifier.
 *
 * @package WP-Authress
 *
 * @since 4.0.0
...
/**
 * Class WP_Authress_IdTokenVerifier
 *
 * @codeCoverageIgnore - Classes are adapted from the PHP SDK and tested there.
 */
final class WP_Authress_IdTokenVerifier {
	 ...
	 *
	 * @var WP_Authress_SignatureVerifier
	 */
	private $verifier;
	 ...
	 * @param WP_Authress_SignatureVerifier $verifier Token signature verifier.
	 */
	public function __construct( string $issuer, string $audience, WP_Authress_SignatureVerifier $verifier ) {
		$this->issuer   = $issuer;
	 ...
	 * @return array
	 *
	 * @throws WP_Authress_InvalidIdTokenException Thrown if:
	...
	public function verify( string $token, array $options = [] ) : array {
		if ( empty( $token ) ) {
			throw new WP_Authress_InvalidIdTokenException( 'ID token is required but missing' );
		}
		...
		if ( ! $tokenIss || ! is_string( $tokenIss ) ) {
			throw new WP_Authress_InvalidIdTokenException( 'Issuer (iss) claim must be a string present in the ID token' );
		}
		...
		if ( ! $tokenSub || ! is_string( $tokenSub ) ) {
			throw new WP_Authress_InvalidIdTokenException( 'Subject (sub) claim must be a string present in the ID token' );
		}
		...
		if ( ! $tokenExp || ! is_int( $tokenExp ) ) {
			throw new WP_Authress_InvalidIdTokenException( 'Expiration Time (exp) claim must be a number present in the ID token' );
		}
		...
		if ( ! $tokenIat || ! is_int( $tokenIat ) ) {
			throw new WP_Authress_InvalidIdTokenException( 'Issued At (iat) claim must be a number present in the ID token' );
		}
		...
		if ( ! empty( $options['nonce'] ) ) {
			...
			if ( ! $tokenNonce || ! is_string( $tokenNonce ) ) {
				throw new WP_Authress_InvalidIdTokenException( 'Nonce (nonce) claim must be a string present in the ID token' );
			}
		...
		if ( null !== $expectedOrganization && '' !== $expectedOrganization ) {
			if ( ! isset( $claims['org_id'] ) || ! is_string( $claims['org_id'] ) ) {
				throw new WP_Authress_InvalidIdTokenException( 'Organization Id (org_id) claim must be a string present in the ID token' );
			}
