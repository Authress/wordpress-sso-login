...
class WP_Authress_JwksFetcher {
	...
	/**
	 * WP_Authress_JwksFetcher constructor.
	 */
	public function __construct() {
		 $this->options = WP_Authress_Options::Instance();
	}
	...
	public function getKeys( $use_cache = true ) : array {
		$keys = $use_cache ? get_transient( WP_AUTHRESS_JWKS_CACHE_TRANSIENT_NAME ) : [];
		...
		if ( $keys && $cache_expiration ) {
			set_transient( WP_AUTHRESS_JWKS_CACHE_TRANSIENT_NAME, $keys, $cache_expiration * MINUTE_IN_SECONDS );
		}
	...
	protected function requestJwks() : array {
		return ( new WP_Authress_Api_Get_Jwks( $this->options, $this->options->get_auth_domain() ) )->call();
	}
}
