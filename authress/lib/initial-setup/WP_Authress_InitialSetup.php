...
class WP_Authress_InitialSetup {
	...
	public function __construct( WP_Authress_Options $a0_options ) {
		...
		$this->connection_profile = new WP_Authress_InitialSetup_ConnectionProfile( $this->a0_options );
			  ...
			  <p>
				  <strong>
					<?php _e( 'There was an error creating the Authress App. Check the ', 'wp-authress' ); ?>
					  <a target="_blank" href="<?php echo admin_url( 'admin.php?page=authress-errors' ); ?>"><?php _e( 'error log', 'wp-authress' ); ?></a>
	...
	public static function get_setup_redirect_uri() {
		return admin_url( 'admin.php?page=authress_setup&callback=1' );
	}
}
