<?php
/**
 * Contains WP_Authress_Api_Client_Credentials.
 *
 * @package WP-Authress
 *
 * @since 3.8.0
...
/**
 * Class WP_Authress_Api_Client_Credentials to perform a client credentials grant.
 */
class WP_Authress_Api_Client_Credentials extends WP_Authress_Api_Abstract {
	 ...
	 */
	const TOKEN_TRANSIENT_KEY = 'authress_api_token';

	/**
	 * Transient key for API token scope.
	 */
	const SCOPE_TRANSIENT_KEY = 'authress_api_token_scope';

	/**
	 * WP_Authress_Api_Client_Credentials constructor.
	 *
	...
	protected function handle_response( $method ) {
		...
		if ( ! isset( $response_body['access_token'] ) ) {
			WP_Authress_ErrorLog::insert_error( $method, __( 'No access_token returned.', 'wp-authress' ) );
			return self::RETURN_ON_FAILURE;
		}
