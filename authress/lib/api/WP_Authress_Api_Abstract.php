<?php
/**
 * Contains WP_Authress_Api_Abstract.
 *
 * @package WP-Authress
 *
 * @since 3.8.0
...
/**
 * Class WP_Authress_Api_Abstract for API calls.
 */
abstract class WP_Authress_Api_Abstract {

	/**
	 * WP cache key.
	 * Used in combination with WP_AUTHRESS_CACHE_GROUP for namespacing.
	 */
	...
	/**
	 * WP_Authress_Api_Abstract constructor.
	 *
	 * @param WP_Authress_Options $options - WP_Authress_Options instance.
	 * @param string|null      $domain - Domain to use.
	 */
	public function __construct( WP_Authress_Options $options, $domain = null ) {
	...
	public static function get_info_headers() {
		$header_value = [
			'name'    => 'wp-authress',
			'version' => WP_AUTHRESS_VERSION,
			'env'     => [
				'php' => phpversion(),
				'wp'  => get_bloginfo( 'version' ),
			],
		];
		return [ 'Authress-Client' => base64_encode( wp_json_encode( $header_value ) ) ];
	}
	...
	protected function set_bearer( $scope ) {
		...
		if ( ! $this->api_client_creds instanceof WP_Authress_Api_Client_Credentials ) {
			return false;
		}
		...
		WP_Authress_ErrorLog::insert_error(
			...
			new WP_Error(
				...
				sprintf( __( 'API token does not include the scope %s.', 'wp-authress' ), $scope )
			)
		);
	...
	protected function handle_wp_error( $method ) {
		if ( $this->response instanceof WP_Error ) {
			WP_Authress_ErrorLog::insert_error( $method, $this->response );
			return true;
		}
		return false;
	}
	...
	protected function handle_failed_response( $method, $success_code = 200 ) {
		...
		$message       = __( 'Error returned', 'wp-authress' );
		...
		if ( isset( $response_body['statusCode'] ) ) {
			...
			WP_Authress_ErrorLog::insert_error( $method, new WP_Error( $response_body['statusCode'], $message ) );
			return true;
		}
		...
		if ( isset( $response_body['error'] ) ) {
			...
			WP_Authress_ErrorLog::insert_error( $method, new WP_Error( $response_body['error'], $message ) );
			return true;
		}

		WP_Authress_ErrorLog::insert_error( $method, $this->response_body );
		return true;
	}
